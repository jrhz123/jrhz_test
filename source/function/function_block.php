...
function block_updatecache($bid, $forceupdate=false) {
	...
	if(is_object($obj)) {
		...
		// Enicn_d 修改 2015-08-10 添加event_event
		//if(in_array($block['blockclass'], array('forum_thread', 'group_thread', 'space_blog', 'space_pic', 'portal_article'))) {
		if(in_array($block['blockclass'], array('forum_thread', 'group_thread', 'space_blog', 'space_pic', 'portal_article', 'event_event'))) {
			$datalist = array();
			// Enicn_d 修改 2015-08-10 添加event_event
			//$mapping = array('forum_thread'=>'tid', 'group_thread'=>'tid', 'space_blog'=>'blogid', 'space_blog'=>'picid', 'portal_article'=>'aid');
			$mapping = array('forum_thread'=>'tid', 'group_thread'=>'tid', 'space_blog'=>'blogid', 'space_blog'=>'picid', 'portal_article'=>'aid', 'event_event'=>'tid');
...
function block_updateitem($bid, $items=array()) {
	global $_G;
	$block = $_G['block'][$bid];
	//Enicn_d 修改2015-08-11 添加浏览数更新
	$obj = block_script($block['blockclass'], $block['script']); //创建模块对象
	$thestyle = !empty($block['styleid']) ? block_getstyle($block['styleid']) : dunserialize($block['blockstyle']); //定义模块设置
	if(!$block) {
	...
	foreach(C::t('common_block_item')->fetch_all_by_bid($bid, true) as $value) {
        //Enicn_d 2015-09-01 按不同模块，读取更新变量 修改编辑类
        $fields = unserialize($value['fields']);
        if ($block['blockclass']=='event_event') {
            $new_value = $obj->getdata($thestyle, array('tids'=>$value['id']));
            $new_fields = $new_value['data'][0]['fields'];
            $fields['status'] = $new_fields['status'];
            $fields['status_style'] = $new_fields['status_style'];
            $fields['bm'] = $new_fields['bm'];
        } elseif ($block['blockclass']=='space_blog') {
            $new_value = $obj->getdata($thestyle, array('blogids'=>$value['id']));
            $new_fields = $new_value['data'][0]['fields'];
            $fields['replynum'] = $new_fields['replynum'];
        }
	...
	for($i = 1; $i <= $block['shownum']; $i++) {
		...
		if(!$jump) {
	        ...
	        //Enicn_d 2015-09-01 按不同模块，读取更新变量 修改非编辑类
	        $fields = unserialize($curitem['fields']);
	        if ($block['blockclass']=='event_event') {
	            $new_value = $obj->getdata($thestyle, array('tids'=>$curitem['id']));
	            $new_fields = $new_value['data'][0]['fields'];
	            $fields['status'] = $new_fields['status'];
	            $fields['status_style'] = $new_fields['status_style'];
	            $fields['bm'] = $new_fields['bm'];
	        } elseif ($block['blockclass']=='space_blog') {
	            $new_value = $obj->getdata($thestyle, array('blogids'=>$curitem['id']));
	            $new_fields = $new_value['data'][0]['fields'];
	            $fields['replynum'] = $new_fields['replynum'];
	            $fields['viewnum'] = $new_fields['viewnum'];
	        } elseif ($block['blockclass']=='forum_thread') {
	        	$new_value = $obj->getdata($thestyle, array('tids'=>$curitem['id']));
	            $new_fields = $new_value['data'][0]['fields'];
	            $fields['replies'] = $new_fields['replies'];
	            $fields['views'] = $new_fields['views'];
	            $fields['heats'] = $new_fields['heats'];
	        }
	        $curitem['fields'] = serialize($fields);
	        //
...
function block_isrecommendable($block) {
	//Enicn_d modified 2015-07-09 add event recommending support
	...
	return !empty($block) && in_array($block['blockclass'], array('forum_thread', 'group_thread', 'portal_article', 'space_pic', 'space_blog', 'event_event')) ? true : false;
}

...
if (!empty($_G['verify_needed'])) {  //如需实名认证，则不允许快速发帖 Enicn_d modified 2017-09-27
	$allowfastpost = false;
}
			...
			if($_G['forum_thread']['addviews']) {
				if($_G['forum_thread']['addviews'] < 100) {
					...
					if(!discuz_process::islocked('update_thread_view')) {
						...
						//Enicn_d modified 2015-04-02 11:35 views times 1
						C::t('forum_thread')->increase($_G['tid'], array('views' => intval($row['addviews'])+1), true);

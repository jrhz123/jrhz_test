...
function insertImage(image, url, width, height) {
	...
	//Enicn_d modified 2015-04-02 11:08 deleted target="_blank"
	var html = '<p><a href="' + url + '"><img src="'+image+'"'+(width?' width="'+width+'"':'')+(height?' height="'+height+'"':'')+'></a></p>';
	edit_insert(html);
}

function insertFile(file, url) {
	url = typeof url == 'undefined' || url === null ? image : url;
	var html = '<p><a href="' + url + '" class="attach">' + file + '</a></p>';
	edit_insert(html);
}

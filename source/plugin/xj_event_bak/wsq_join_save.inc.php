...
if($_GET['action']=='cannel'){
	...
	if($items['activityexpiration']>$timestamp){
		...
		showmessage(lang('plugin/xj_event', 'cgqxbm'),"http://wsq.discuz.qq.com/?c=index&a=viewthread&f=wx&tid={$tid}&siteid={$siteid}&_bpage=1");
	}else{
		exit('Access Denied');
	}
}
...
if($setting['noverify']==1){
	if($setting['eventpay']){
		...
		showmessage(lang('plugin/xj_event', 'gxnbmcg'), "http://wsq.discuz.qq.com/?c=index&a=viewthread&f=wx&tid={$tid}&siteid={$siteid}&_bpage=1", array(), array('showdialog' => 1, 'showmsg' => true, 'locationtime' => true, 'alert' => 'right'));
	}
}else{
	if($setting['eventpay']){
		showmessage(lang('plugin/xj_event', 'bmzltjcgxzzrzfym'), "plugin.php?id=xj_event:event_pay&tid=$tid", array(), array('showdialog' => 1, 'showmsg' => true, 'locationtime' => true, 'alert' => 'right'));
	...
	showmessage(lang('plugin/xj_event', 'bmxxtjcgqddsh'), "http://wsq.discuz.qq.com/?c=index&a=viewthread&f=wx&tid={$tid}&siteid={$siteid}&_bpage=1", array(), array('showdialog' => 1, 'showmsg' => true, 'locationtime' => true, 'alert' => 'right'));
}
?>
...
function logincheck($username) {
	...
	if(function_exists('uc_user_logincheck')) {
		...
		/**
		Enicn_d modified 2015-04-09 failedlogin 15 seconds
		*/
		$return = (!$login || (TIMESTAMP - $login['lastupdate'] > 15)) ? 5 : max(0, 5 - $login['count']);
		...
		} elseif(TIMESTAMP - $login['lastupdate'] > 15) {
			C::t('common_failedlogin')->insert(array(
				'ip' => $_G['clientip'],
				'count' => 0,
				'lastupdate' => TIMESTAMP

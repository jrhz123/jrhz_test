...
if($_GET['operation'] == 'upload') {
	...
	if (!empty($_G['gp_pid'])) {
		$count_p = C::t('forum_attachment_n')->count_by_id('tid:'.$_G['gp_tid'], 'pid', $_G['gp_pid']);
		$count_u = C::t('forum_attachment_n')->count_by_id(127, 'uid', $_G['uid']);
		$count = $count_p + $count_u;
		$postdata = C::t('forum_post')->fetch_by_pid_condition('tid:'.$_G['gp_tid'], $_G['gp_pid']);
		if ($postdata['first']!=1) {
			$is_first = false;
		} else {
			$is_first = true;
		...
		$count = C::t('forum_attachment_n')->count_by_id(127, 'uid', $_G['uid']);
		$is_first = true;
	}
	if ($is_first) {
		$count_limit = 8;
	...
	if ($count < $count_limit)  { //Enicn_d modified 2015-11-30 增加图片数量限制

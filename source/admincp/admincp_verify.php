...
$anchor = in_array($_GET['anchor'], array('base', 'edit', 'verify', 'verify1', 'verify2', 'verify3', 'verify4', 'verify5', 'verify6', 'verify7', 'verify8', 'authstr', 'refusal', 'pass')) ? $_GET['anchor'] : 'base'; //modified by Enicn_d 2017-03-03 增加认证类型
...
if($operation == 'verify') {
	...
	$current = array($anchor => 1);
	if($anchor == 'add') {
		//modified by Enicn_d 2017-03-03 增加认证类型 将 $vid > 7 改成 $vid > 8
		if(!submitcheck('addverifysubmit') || $vid < 0 || $vid > 8) {
...
EOF;
			...
			$intkeys = array('uid', 'verifytype', 'flag', 'verify1', 'verify2', 'verify3', 'verify4', 'verify5', 'verify6', 'verify7', 'verify8'); //modified by Enicn_d 2017-03-03 增加认证类型 verify8
			$strkeys = array();
			$randkeys = array();
	...
	//modified by Enicn_d 2017-03-03 增加认证类型 将 $_GET['vid'] < 8 改为 $_GET['vid'] < 9
	$vid = $_GET['vid'] < 9 ? intval($_GET['vid']) : 0;
	...
	if(!submitcheck('verifysubmit')) {
		...
		//modified by Enicn_d 2017-03-03 增加认证类型 将 $i < 8 改为 $i < 9
		for($i = 1; $i < 9; $i++) {

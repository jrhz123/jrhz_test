...
if($_GET['act']=='cj'){
	$listcount = DB::result_first("SELECT count(*) FROM ".DB::table('xj_eventapply')." A,".DB::table('xj_event')." B,".DB::table('forum_thread')." C WHERE A.tid=B.tid and B.tid=C.tid and A.uid=".intval($_G['uid']));
	$page = $_GET['page']?$_GET['page']:1;
	//if(@ceil($listcount/$perpage) < $page) {
	//	$page = 1;
	//}
	$start_limit = ($page - 1) * $perpage;
	$query = DB::query("SELECT * FROM ".DB::table('xj_eventapply')." A,".DB::table('xj_event')." B,".DB::table('forum_thread')." C WHERE A.tid=B.tid and B.tid=C.tid AND A.uid=".intval($_G['uid'])." ORDER BY B.eid DESC LIMIT $start_limit,$perpage");
}elseif($_GET['act']=='zz'){
	$listcount = DB::result_first("SELECT count(*) FROM ".DB::table('xj_event')." A,".DB::table('forum_thread')." B WHERE A.tid=B.tid and B.authorid=".intval($_G['uid'])." ORDER BY A.eid");
	$page = $_GET['page']?$_GET['page']:1;
	//if(@ceil($listcount/$perpage) < $page) {
	//	$page = 1;
	//}
	$start_limit = ($page - 1) * $perpage;
	$query = DB::query("SELECT * FROM ".DB::table('xj_event')." A,".DB::table('forum_thread')." B WHERE A.tid=B.tid and B.authorid=".intval($_G['uid'])." ORDER BY A.eid DESC LIMIT $start_limit,$perpage");
}
...
<script>
        var menu = new Array();
		menu.push({name:"<?php echo lang('plugin/xj_event', 'changjia'); ?>", pluginid:'xj_event:wsq_my_event', param:"act=cj"});
        menu.push({name:"<?php echo lang('plugin/xj_event', 'zuzhi'); ?>", pluginid:'xj_event:wsq_my_event', param:"act=zz"});
        WSQ.initBtmBar(menu);
        WSQ.showBtmBar();
        WSQ.initPlugin({name:'<?php echo lang('plugin/xj_event', 'myevent'); ?>'});
...
<div id="mycontainer">
	...
	<div style="background-color:#FFFFFF;-moz-border-radius: 8px;-webkit-border-radius: 8px;border-radius:8px; padding:10px; margin-bottom:8px; border-bottom:2px #ddd solid;" onClick="window.open('http://wsq.discuz.qq.com/?c=index&a=viewthread&f=wx&tid=<?php echo $value['tid']; ?>&siteid=<?php echo $siteid; ?>&_bpage=1','_parent');">

...
/**
Enicn_d **** 2014.10.31 **** Added 'shuji','bumen','duban','paotui','unreply' | 2015.03.02 Added 'zhuti'
*/
$filterfield_rc = array('zhuti','shuji','bumen','duban','paotui','unreply');
$filterfield = array_merge($filterfield, $filterfield_rc);
/**********************************************************************************************************************************************************************/
...
/**
Enicn_d ***Customized threadcount
*/
/**************************************************************************** Customized Part *************************************************************************/
	...
	if (in_array($filter, $filterfield_rc) && $_G['fid']==$target_forum) {
		$order_by = isset($_GET['orderby']) ? $_GET['orderby'] : 'lastpost';
		$sort_by = isset($_GET['ascdesc']) ? $_GET['ascdesc'] : 'DESC';
		$orderation = "ORDER BY `$order_by` $sort_by";

		$offset = ($_G['page'] - 1) * $_G['tpp'];
		$limit = $_G['tpp'];
		$limitation = "LIMIT $offset, $limit";
		switch ($filter) {
			case 'zhuti': //书记发表的帖子
				$target_users = $C['target_users'];
				/*displayorder=0时，表示正常帖子,displayorder=-1时，被删除的帖子,displayorder=-2时，表示待审核的帖子,displayorder=1时，表示板块置顶帖子,displayorder=2时，表示分类置顶帖子,displayorder=3时，表示全局置顶帖子 现要显示全部置顶贴和普通贴*/
				$condition = "AND `authorid` IN ($target_users) AND `displayorder` = 0";
				$counting_string = "SELECT COUNT(*) AS num
										FROM ".DB::table('forum_thread')."
										WHERE `fid`=$target_forum $condition";
				$query_string = "SELECT *
									FROM ".DB::table('forum_thread')."
			...
			case 'shuji': //书记批示过的帖子
				$target_users = $C['target_users'];
				$condition = "AND (`displayorder` >= 0 OR `displayorder` = -5) AND `first` = 0";  //Enicn_d modified 2017-03-30 增加显示沉底帖子
				$counting_string = "SELECT COUNT(DISTINCT t.`tid`) AS num
										FROM ".DB::table('forum_thread')." t
										RIGHT JOIN ".DB::table('forum_post')." p
											ON t.`tid` = p.`tid`
										WHERE p.`authorid` IN ($target_users)
											AND p.`fid`=$target_forum
											$condition";
			...
			case 'bumen': //部门回应过的帖子
				...
				$condition = "AND `displayorder` >= 0 AND `first` = 0";
				$counting_string = "SELECT COUNT(DISTINCT t.`tid`) AS num
										FROM ".DB::table('forum_thread')." t
										RIGHT JOIN ".DB::table('forum_post')." p
											ON t.`tid` = p.`tid`
										WHERE p.`authorid` IN ($group_uids)
											AND p.`fid`=$target_forum
											$condition";
				$query_string = "SELECT t.*
									FROM ".DB::table('forum_thread')." t
			...
			case 'duban': //督办帖子
				...
				$condition = "AND `authorid` IN ($vice_users) AND `displayorder` >= 0";
				$counting_string = "SELECT COUNT(*) AS num
										FROM ".DB::table('forum_thread')."
										WHERE `fid`=$target_forum
										$condition";
			...
			case 'paotui': //网记跑腿帖子
				...
				$group_uids = '';
				$paotui_tids = '';
				$vice_groups = unserialize($C['vice_groups']);
				$groupids = implode(',', $vice_groups);
				$g_uids = DB::fetch_all("SELECT uid FROM ".DB::table('common_member')." WHERE `groupid` IN ($groupids)");
				if (is_array($g_uids)) {
					foreach ($g_uids as $key => $value) {
						$group_uids .= $value['uid'].',';
				...
				$query_string = "SELECT *
									...
									$limitation";
				break;
			case 'unreply': //部门与书记均未回复的帖子（不包括部门或书记发布的帖子）
				$groupids = '';
				$group_uids = '';
				$replied_tids = '';
				$replied_tids_limited = array();
				$target_groups = unserialize($C['target_groups']);
				$groupids = implode(',', $target_groups);
				$g_uids = DB::fetch_all("SELECT uid FROM ".DB::table('common_member')." WHERE `groupid` IN ($groupids)");
				if (is_array($g_uids)) {
				...
				if (empty($C['target_users'])) {
					$group_uids = substr($group_uids, 0, -1);
				} else {
					$group_uids .= $C['target_users'];
				}
				$r_tids = DB::fetch_all("SELECT tid FROM ".DB::table('forum_post')." WHERE `authorid` IN ($group_uids)");
				if (is_array($r_tids)) {
					foreach ($r_tids as $key => $value) {
						$replied_tids .= $value['tid'].',';
						if ($key<$limit) {
							$replied_tids_limited[] = $value['tid'];
						}
					}
				}
				$replied_tids = substr($replied_tids, 0, -1);
				$replied_tids_limited = implode(',', $replied_tids_limited);
				if (!empty($replied_tids)) {
					$condition = "AND `tid` NOT IN ($replied_tids) AND `displayorder` >= 0";
				}
				$counting_string = "SELECT COUNT(*) AS num
										FROM ".DB::table('forum_thread')."
										WHERE `fid`=$target_forum
											AND `authorid` NOT IN ($group_uids)
											$condition";
				$query_string = "SELECT *
									FROM ".DB::table('forum_thread')."
									WHERE `fid`=$target_forum
				...
				break;
			default:
				$counting_string = "SELECT COUNT(*) AS num FROM ".DB::table('forum_thread')." WHERE `fid`=$target_forum $condition";
				$query_string = "SELECT * FROM ".DB::table('forum_thread')." WHERE `fid`=$target_forum $orderation $limitation";
				break;
		}
		$_G['forum_threadcount'] = (int) DB::result_first($counting_string);
	} else {
/*************************************************************************** Origin Part ******************************************************************************/
		$filterarr['sticky'] = 0;
...
$filterarr['displayorder'] = !$filterbool && $stickycount ? array(0, 1, -5) : array(0, 1, 2, 3, 4, -5); //Enicn_d 修改 2016-03-08 修改帖子下沉功能

if($filter !== 'hot') {
/**
Customized threadlist
*/
/**************************************************************************** Customized Part *************************************************************************/
	if (in_array($filter, $filterfield_rc) && $_G['fid']==$target_forum) {
		$threadlist = array();
		$filterarr1 = $filterarr;
		$filterarr1['displayorder'] = array(1, 2, 3, 4);
		$filterarr1['fid'] = $_G['fid'];
		$threadlist = C::t('forum_thread')->fetch_all_search($filterarr1, $tableid, $start_limit, $_G['tpp'], 'lastpost');
		unset($filterarr1);
		$threadlist = array_merge($threadlist, DB::fetch_all($query_string));
	} else {
/****************************************************************************** Origin Part ***************************************************************************/
		$threadlist = array();
		$indexadd = '';
		$_order = "displayorder DESC, $_GET[orderby] $_GET[ascdesc]";
		if (defined('IN_MOBILE')) { //Enicn_d 修改 2015-09-08 手机版允许显示置顶帖子
			$filterarr['sticky'] = 0;
			$showsticky = false;
			$_order = 'lastpost DESC';
		}
		if($filterbool) {
			if($filterarr['digest']) {
...
$threadindex = 0;
$flag_separate_stop = false; //Enicn_d 2016-05-06 修改 增加置顶帖子停止计算标记
foreach($threadlist as $thread) {
	...
	if(in_array($thread['displayorder'], array(1, 2, 3, 4)) && !$flag_separate_stop) { //Enicn_d 2016-05-06 修改 增加置顶帖子停止计算标记
		$thread['id'] = 'stickthread_'.$thread['tid'];
		$separatepos++;
	} else {
		$flag_separate_stop = true; //Enicn_d 2016-05-06 修改 增加置顶帖子停止计算标记
...
if($_G['forum']['livetid'] && $page == 1 && (!$filter || ($filter == 'sortid' && $_G['forum']['threadsorts']['defaultshow'] == $_GET['sortid']))) {
	...
	//Enicn_d modified 2015-05-21 13:18 added thumbs to livethread
	$_G['tid'] = $livethread['tid'];
	$pid = $livepost['pid'];
	$message = $livepost['message'];
	$result = preg_match_all("/\[attach\].*\[\/attach\]/i", $message, $attach);
	$attach = $attach[0];
	foreach ($attach as $key => $value) {
		$attach[$key] = preg_replace("/\[(\/)?attach\]/i", '', $value);
	}
	include_once libfile('function/post');
	include_once libfile('function/discuzcode');
	$attach_list = getattach($pid);
	$attach_data = array();
	foreach ($attach_list['imgattachs']['used'] as $image) {
		$attach_data[$image['aid']] = parseimg(0,120,$image['url'].'/'.$image['attachment'],0,$pid);
	}
	$images_list = array();
	for ($i=0;$i<3;$i++) {
		if ($attach_data[$attach[$i]]) {
			$images_list[] = $attach_data[$attach[$i]];
		}
	}
	/***************************************************************/
}
...
if (!empty($_G['verify_needed'])) {  //如需实名认证，则不允许快速发帖 Enicn_d modified 2017-07-27
	$allowfastpost = false;
}

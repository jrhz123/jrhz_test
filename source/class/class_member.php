...
class register_ctl {
	...
	function on_register() {
		...
		if($this->setting['regverify']) {
			...
			$groupinfo['groupid'] = 18;//Edit by stellar 2017-9-30 $this->setting['newusergroupid'];
		}
		...
		if(!submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck)) {
			...
			if (!empty($_G['verify_needed'])) { //短信验证 stellar modified 2017-09-29
				$mobile = $_POST['mobile'];
				$code = $_POST['verifycode'];
				$tablename = DB::table('common_verifycodes');
				if (empty($code)) {showmessage('complete_info');exit();}
				...
				if (empty($check) || time() - $check['sendtime'] > 120) {
					showmessage('phone_code');
					exit();
				} else {
					$verify_passed = TRUE;
				}
			}//短信验证 stellar modified 2017-09-29
			...
			if (!empty($verify_passed)) { //验证通过后直接通过审核 stellar modified 2017-09-29
				C::t('common_member_verify')->insert(array('uid' => $uid,'verify6' => 1 ));
				C::t('common_member_profile')->update($uid, array('realname'=>$_POST['realname'], 'mobile'=>$_POST['mobile']));
			}//验证通过后直接通过审核 stellar modified 2017-09-29

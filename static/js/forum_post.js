...
var STATUSMSG = {
	...
	'11' : '今日您已无法上传那么大的附件',
	'12' : '单个楼层不可超过10张图片',
	'13' : '单次提交不可超过10张图片，请先删除未使用的图片，或直接使用未使用的图片',
	'14' : '顶楼不可超过8张图片',
	'15' : '顶楼不可超过8张图片，请先删除未使用的图片，或直接使用未使用的图片'
};
//Enicn_d modified 2015-12-01 添加图片数量限制
...
function validate(theform) {
	...
	//Enicn_d modified 2015-04-23 17:11 added maxlen
	maxlen = 30;
	if(($('postsubmit').name != 'replysubmit' && !($('postsubmit').name == 'editsubmit' && !isfirstpost) && theform.subject.value == "") || !sortid && !special && trim(message) == "") {
		showError('抱歉，您尚未输入标题或内容');
		return false;
	} else if(theform.subject.value.length > maxlen) {
		showError('您的标题超过 '+maxlen+' 个字符的限制');
		return false;
	}
	...
	if(!op) {
		...
		if(aids) {
			if(type == 'attach') {
                ...
                //Enicn_d modified 2015-12-02 清空ATTACHNUM['imageunused_halt']，配合图片限制的提示
                ATTACHNUM['imageunused_halt'] = 0;
			}
		}

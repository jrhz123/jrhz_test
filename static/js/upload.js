			...
			if(this.customSettings.uploadType == 'poll') {
				...
				if(aid > 0) {
					...
					if(typeof STATUSMSG[aid] == "string") {
						...
						if (aid==12) {
							if (ATTACHNUM['imageunused_halt']>0) { //Enicn_d modified 2015-12-02 添加未使用图片提示
								showDialog(STATUSMSG[13], 'notice', null, null, 0, null, null, null, null, 5);
								$('attachnotice_img').style.border = "3px solid red";
								hideAttachMenu('image');
							} else {
								showDialog(STATUSMSG[12], 'notice', null, null, 0, null, null, null, null, sdCloseTime);
							}
						} else if(aid==14) {
							if (ATTACHNUM['imageunused_halt']>0) { //Enicn_d modified 2015-12-02 添加未使用图片提示
								showDialog(STATUSMSG[15], 'notice', null, null, 0, null, null, null, null, sdCloseTime);
								$('attachnotice_img').style.border = "3px solid red";
								hideAttachMenu('image');
							} else {
								showDialog(STATUSMSG[14], 'notice', null, null, 0, null, null, null, null, sdCloseTime);
							}
						} else {

...
class discuz_application extends discuz_base{
	...
	private function _init_user() {
		...
		define('REALNAME_RESTRICT', TRUE);  //实名制开关 Enicn_d modified 2017-09-27
		$uid = $this->var['member']['uid'];
		if (REALNAME_RESTRICT) { //实名认证限制 Enicn_d modified 2017-09-27
			$tablename = DB::table('common_member_verify');
			$check = DB::fetch_first("SELECT * FROM {$tablename} WHERE `uid` = {$uid}");
			if ( empty($check) || empty($check['verify6']) ) {
				setglobal('verify_needed', TRUE);  //标记表示用户需进行实名认证
			}
		}
	}
	...
	private function _init_mobile() {
		...
		if(!$this->var['mobile'] && !$unallowmobile) {
			if($mobileflag) {
				...
				// var_dump($this->var['mobile']);
				// var_dump($unallowmobile);
				// var_dump($mobileflag);
				// exit;
			}
		}

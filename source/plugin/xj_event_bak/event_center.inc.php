...
if($_G['cache']['plugin']['xj_event']['event_sysevent']){
	require_once libfile('function/post');
	$query = DB::query("SELECT A.tid,A.aid,A.starttimefrom,A.expiration,B.message,B.subject FROM ".DB::table('forum_activity')." A,".DB::table('forum_post')." B,".DB::table('forum_thread')." C WHERE A.tid = B.tid AND B.tid=C.tid AND C.displayorder >= 0 AND B.first=1  ORDER BY A.starttimefrom DESC LIMIT 0,12");
	$syseventlist = array();
	while($value = DB::fetch($query)){
		$value['message'] = messagecutstr($value['message'],50);
...
if($_G['cache']['plugin']['xj_event']['event_centerstyle']==2){
	...
	$query = DB::query("SELECT A.tid,A.starttime,A.endtime,A.use_cost,A.event_address,A.activityaid,A.activityaid_url,A.activityexpiration,B.subject,B.message FROM ".DB::table('xj_event')." A,".DB::table('forum_post')." B,".DB::table('forum_thread')." C WHERE A.tid = B.tid AND B.tid = C.tid AND C.displayorder >= 0 AND B.first=1 AND A.activitybegin<$nowtime AND A.endtime>$nowtime ORDER BY A.activityexpiration DESC");
	$eventlist = array();
	while($value = DB::fetch($query)){
	...
	if($eventcount>0){
		$query = DB::query("SELECT A.tid,A.starttime,A.endtime,A.use_cost,A.event_address,A.activityaid,A.activityaid_url,A.activityexpiration,B.subject,B.message FROM ".DB::table('xj_event')." A,".DB::table('forum_post')." B WHERE A.tid = B.tid AND B.first=1 AND A.endtime<$nowtime ORDER BY A.activityexpiration DESC LIMIT 0,$eventcount");
		while($value = DB::fetch($query)){
			$value['applynumber'] = DB::result_first("SELECT SUM(applynumber) FROM ".DB::table('xj_eventapply')." WHERE tid=".$value['tid']." and verify=1");
			$value['applynumber'] = $value['applynumber']?$value['applynumber']:0;
     ...
     /* 史漫漫 2015年4月3日修改*/
	$query = DB::query("SELECT A.tid,A.starttime,A.endtime,A.use_cost,A.event_address,A.activityaid,A.activityaid_url,B.subject,B.message,A.activitybegin,A.activityexpiration FROM ".DB::table('xj_event')." A,".DB::table('forum_post')." B,".DB::table('forum_thread')." C WHERE A.tid = B.tid AND B.tid = C.tid AND C.displayorder >= 0 AND B.first=1 AND A.activitybegin>$nowtime ORDER BY eid DESC");
	$ygeventlist = array();
	while($value = DB::fetch($query)){

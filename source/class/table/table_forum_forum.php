...
class table_forum_forum extends discuz_table
{
	...
	public function update_forum_counter($fid, $threads = 0, $posts = 0, $todayposts = 0, $modwork = 0, $favtimes = 0) {
		...
		$addsql = array();
		/**
		Enicn_d modified 2015-04-08 11:44 added times 3
		*/
		if (rand(0,9)>0 && $fid != 863) {
			$times = 3;
		} else {
			$times = 1;
		}
		if($threads) {
			$threads = intval($threads) * $times;
			$addsql[] = "threads=threads+'".$threads."'";
		}
		if($posts) {
			$posts = intval($posts) * $times;
			$addsql[] = "posts=posts+'".$posts."'";
		}
		if($todayposts) {
			$todayposts = intval($todayposts) * $times;
			$addsql[] = "todayposts=todayposts+'".$todayposts."'";
		}
		if($modwork) {
			$addsql[] = "modworks='1'";
		}

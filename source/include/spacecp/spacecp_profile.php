...
if(submitcheck('profilesubmit')) {
	...
	if (!empty($_G['verify_needed'])) { //短信验证 Enicn_d modified 2017-09-28
		$mobile = $_GET['mobile'];
		$code = $_GET['verifycode'];
		$tablename = DB::table('common_verifycodes');
		$check = DB::fetch_first("SELECT * FROM {$tablename} WHERE `phone` = {$mobile} AND `code` = {$code} LIMIT 1");
		if (empty($check) || time() - $check['sendtime'] > 120) {
			profile_showerror('verifycode', '短信码不正确或已过期');
		} else {
			$verify_passed = TRUE;
		}
	}
	...
	if($verifyarr) {
		...
		if (!empty($verify_passed)) { //验证通过后直接通过审核 Enicn_d modified 2017-09-28
			C::t('common_member_verify')->update($_G['uid'], array("verify{$vid}" => 1));
			$verify_info = C::t('common_member_verify_info')->fetch_by_uid_verifytype($_G['uid'], $vid);
			C::t('common_member_verify_info')->update($verify_info['vid'], array("flag" => 1));
			C::t('common_member_profile')->update($_G['uid'], array('realname'=>$verifyarr['realname'], 'mobile'=>$verifyarr['mobile']));
		}
...
if($operation == 'password') {
	...
	if(!empty($verify) && is_array($verify)) {
		foreach($verify as $key => $flag) {
			//modified by Enicn_d 2017-03-03 增加认证类型 verify8
			if(in_array($key, array('verify1', 'verify2', 'verify3', 'verify4', 'verify5', 'verify6', 'verify7', 'verify8')) && $flag == 1) {
...
function profile_showerror($key, $extrainfo) {
	if (checkmobile()) { //移动端判断 Enicn_d modified 2017-09-28
		showmessage($extrainfo);
	} else {
		echo '<script>';

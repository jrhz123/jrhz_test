...
if($apitype == 'alipay'){
	...
	if($notifydata['trade_status'] == 'WAIT_BUYER_PAY'){   //等待支付
		...
		$item = DB::fetch_first("SELECT applyid,costtype FROM ".DB::table('xj_eventpay_log')." WHERE orderid = '$orderid'"); //Enicn_d 2015-11-03 added costtype, reserve_cost
		$applyid = $item['applyid'];
		if ($item['costtype']=='reserve_cost') {
			DB::query("UPDATE ".DB::table('xj_eventapply')." SET pay_state=2 WHERE applyid = $applyid");
		} else {
		...
		$item = DB::fetch_first("SELECT tid,applyid,costtype FROM ".DB::table('xj_eventpay_log')." WHERE orderid = '$orderid'"); //Enicn_d 2015-11-03 added costtype, reserve_cost
		$applyid = $item['applyid'];
        $tid = $item['tid'];
        $event = DB::fetch_first("SELECT verify_by_pay FROM ".DB::table('xj_event')." WHERE tid = '$tid'"); //Enicn_d 2015-11-03 added verify_by_pay
        if ($event['verify_by_pay']==1) {
    		if ($item['costtype']=='reserve_cost') {
    			DB::query("UPDATE ".DB::table('xj_eventapply')." SET pay_state=2,verify=1 WHERE applyid = $applyid");
    		} else {

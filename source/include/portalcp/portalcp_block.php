...
if($op == 'block') {
	...
	if(submitcheck('blocksubmit')) {
		...
		//Enicn_d 修改 2015-08-10 增加n-d H:i
		//$_POST['dateformat'] = in_array($_POST['dateformat'], array('Y-m-d', 'm-d', 'H:i', 'Y-m-d H:i')) ? $_POST['dateformat'] : 'Y-m-d'; 
		$_POST['dateformat'] = in_array($_POST['dateformat'], array('Y-m-d', 'm-d', 'H:i', 'Y-m-d H:i', 'n-d H:i')) ? $_POST['dateformat'] : 'Y-m-d'; 
...
function block_getdateformats($format='') {
	...
	$formats = array('Y-m-d', 'm-d', 'H:i', 'Y-m-d H:i', 'n-d H:i');
	$return = array();
...
function get_push_item($block, $blockstyle, $id, $idtype, $blockclass = '', $script = '') {
	...
	if(empty($blockclass) || empty($script)) {
		if($idtype == 'tids') {
			if ($blockclass=='event_event') {
				$obj = block_script('event', 'event');
			} else {

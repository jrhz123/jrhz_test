...
';
	...
	if($livetid) {
		...
		foreach($postarr as $post) {
			...
			//Enicn_d modified 2015-05-21 13:16 added thumbs to livelastpost
			...
			foreach ($attach_list['imgattachs']['used'] as $image) {
				$attach_data[] = parseimg(0,120,$image['url'].'/'.$image['attachment'],0,$pid);
			}
			$images_list = array();
			for ($i=0;$i<3;$i++) {
				if ($attach_data[$i]) {
					$images_list[$i] = $attach_data[$i];
				}
			}
			$post_redirect = "/forum.php?mod=redirect&goto=findpost&ptid=$tid&pid=$pid";
			$message = "<p><a href='$post_redirect'>".str_replace("\r\n", '<br>', messagecutstr($post['message']))."</a></p>";

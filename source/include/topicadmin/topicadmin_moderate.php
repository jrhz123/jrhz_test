...
if($_GET['moderate']) {
	foreach($threadtableids as $tableid) {
		foreach(C::t('forum_thread')->fetch_all_by_tid_fid_displayorder($_GET['moderate'], $_G['fid'], null, '', 0, $_G['tpp'], '', '', $tableid) as $thread) {
			...
			if($thread['closed'] > 1 && $operation && !in_array($operation, array('delete', 'highlight', 'stick', 'digest', 'bump', 'down')) || $thread['displayorder'] < 0 && $thread['displayorder'] != -4 && $thread['displayorder'] != -5) { //Enicn_d 修改 2016-03-08 修改帖子下沉功能
	...
	if(empty($operations)) {
		...
		foreach($operations as $operation) {
			...
			if($operation == 'stick') {
				...
				C::t('forum_thread')->update($tidsarr, array('displayorder'=>0, 'moderated'=>1), true); //Enicn_d 修改 2016-03-08 修改帖子下沉功能
				...
				C::t('forum_thread')->update($tidsarr, array('displayorder'=>-5, 'moderated'=>1), true); //Enicn_d 修改 2016-03-08 修改帖子下沉功能

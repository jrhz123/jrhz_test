...
if($_GET['action'] == 'edit' || $_GET['action'] == 'reply') {
	...
	if(!$_G['forum_auditstatuson'] && !(($thread['displayorder']>=0 || $thread['displayorder']==-5) || (in_array($thread['displayorder'], array(-4, -2)) && $thread['authorid']==$_G['uid']))) { //Enicn_d 修改 2016-03-08 修改帖子下沉功能
		$thread = array();
	}
	if(!empty($thread)) {
...
if (!empty($_G['verify_needed']) ) {  //检查是否需要实名认证 Enicn_d modified 2017-09-27
	showmessage('通过实名认证后方可发表主题帖或跟帖', '/home.php?mod=spacecp&ac=profile&op=verify&vid=6');
}

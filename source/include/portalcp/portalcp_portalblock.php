...
if(submitcheck('getblocklistsubmit') || submitcheck('verifieddatasubmit') || submitcheck('verifydatasubmit')) {
	...
	if($op == 'recommend') {
		...
		switch ($_GET['idtype']) {
			case 'tid' :
				//Enicn_d 2015-07-16 修改 添加帖子类型为tid的event_event模块支持
				//$rewhere[] = "(blockclass='forum_thread' OR blockclass='forum_activity' OR blockclass='forum_trade')";
				$rewhere[] = "(blockclass='forum_thread' OR blockclass='forum_activity' OR blockclass='forum_trade' OR blockclass='event_event')";
				break;
			case 'gtid' :

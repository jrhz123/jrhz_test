	...
	if(!empty($tid) || !empty($fid)) {
		...
		if(!empty ($tid)) {
			...
			if(!$_G['forum_auditstatuson'] && !empty($_G['thread'])
					...
					&& !(($_G['thread']['displayorder'] >= 0 || $_G['thread']['displayorder'] == -5) || (in_array($_G['thread']['displayorder'], array(-4,-3,-2)) && $_G['uid'] && $_G['thread']['authorid'] == $_G['uid']))) { //Enicn_d 修改 2016-03-08 修改帖子下沉功能
				$_G['thread'] = null;
			}
...
function threadpubsave($tid, $passapproval = false) {
	...
	//Enicn_d modified 2015-04-03 11:34 threads, posts, todayposts,modwork times 1

...
function discuzcode($message, $smileyoff, $bbcodeoff, $htmlon = 0, $allowsmilies = 1, $allowbbcode = 1, $allowimgcode = 1, $allowhtml = 0, $jammer = 0, $parsetype = '0', $authorid = '0', $allowmediacode = '0', $pid = 0, $lazyload = 0, $pdateline = 0, $first = 0) {
	...
	if(!$bbcodeoff && $allowbbcode) {
		...
		if(!defined('IN_MOBILE')) {
			...
			if(strpos($msglower, '[/media]') !== FALSE) {
				//Enicn_d 修改 支持手机版播放视频
				//$message = preg_replace("/\[media=([\w,]+)\]\s*([^\[\<\r\n]+?)\s*\[\/media\]/is", "[media]\\2[/media]", $message);
				if (strpos($message, 'youku.com') !== FALSE) {
					$message = preg_replace("/\[media=[\w,]+\]\s*.+sid\/(.+)\/v.+\s*\[\/media\]/is", "<iframe height='260' width='100%' src='http://player.youku.com/embed/\\1' frameborder=0 allowfullscreen></iframe>", $message);
				} elseif (strpos($message, 'tudou.com') !== FALSE) {
					 if (strpos($message, '/l/') !== FALSE) {
					 	$message = preg_replace("/\[media=[\w,]+\]\s*.+\/l\/([\w\-]+)\/.*\s*\[\/media\]/is", "<a href='http://www.tudou.com/listplay/\\1/index.html'><img src='static/image/common/vd.gif' /></a>", $message);
					 } elseif (stripos($message, '/v/') !== FALSE) {
					 	$message = preg_replace("/\[media=[\w,]+\]\s*.+\/v\/([\w\-]+)\/.*\s*\[\/media\]/is", "<a href='http://www.tudou.com/programs/view/\\1/'><img src='static/image/common/vd.gif' /></a>", $message);
					 } else {
					 	$message = preg_replace("/\[media=[\w,]+\]\s*([\w\/\.\:\-]+)\s*\[\/media\]/is", "<a href='\\1'><img src='static/image/common/vd.gif' /></a>", $message);
					 }
				} elseif (stripos($message, 'qiyi.com') !== FALSE) {
					$message = preg_replace("/\[media=[\w,]+\]\s*.+\/(\w+)\.swf.*\s*\[\/media\]/is", "<a href='http://www.iqiyi.com/\\1.html'><img src='static/image/common/vd.gif' /></a>", $message);
				} elseif (strpos($message, '163.com') !== FALSE) {
					$message = preg_replace("/\[media=[\w,]+\]\s*.+vid=(\w+).*sid=(\w+).*coverpic=([\w:\/\.]+).*\s*\[\/media\]/is", "<div style='position:relative;'><a href='http://v.163.com/paike/\\2/\\1.html'><img src='\\3' /><img src='static/image/common/vds.png' style='position:absolute; top:50%; left:100px' /></a></div>", $message);
				} elseif (stripos($message, 'qq.com') !== FALSE) {
					$message = preg_replace("/\[media=[\w,]+\]\s*.+vid=(\w+).*\s*\[\/media\]/is", "<iframe frameborder='0' height='260' width='100%' src='http://v.qq.com/iframe/player.html?vid=\\1&tiny=0&auto=0' allowfullscreen></iframe>", $message);
				} else {
					$message = preg_replace("/\[media=[\w,]+\]\s*([^\[\<\r\n]+?)\s*\[\/media\]/is", "<a href='\\1'><span style='color:red;font-weight:bold'>该视频来源不支持外链手机播放，请点击跳转到视频地址播放</span></a>", $message);
				}
			}
			if(strpos($msglower, '[/audio]') !== FALSE) {
				//Enicn_d 修改 支持手机版播放音频
				//$message = preg_replace("/\[audio(=1)*\]\s*([^\[\<\r\n]+?)\s*\[\/audio\]/is", "[media]\\2[/media]", $message);
				$message = preg_replace("/\[audio(=1)*\]\s*([^\[\<\r\n]+?)\s*\[\/audio\]/is", "<embed width='100%' height='260' allownetworking='internal' allowscriptaccess='never' src='\\2' quality='high' bgcolor='#ffffff' wmode='transparent' allowfullscreen='true' type='application/x-shockwave-flash'></embed>", $message);
			}
			if(strpos($msglower, '[/flash]') !== FALSE) {
				//Enicn_d 修改 支持手机版播放视频
				//$message = preg_replace("/\[flash(=(\d+),(\d+))?\]\s*([^\[\<\r\n]+?)\s*\[\/flash\]/is", "[media]\\4[/media]", $message);
				if (strpos($message, 'youku.com') !== FALSE) {
					$message = preg_replace("/\[flash=[\w,]+\]\s*.+sid\/(.+)\/v.+\s*\[\/flash\]/is", "<iframe height='260' width='100%' src='http://player.youku.com/embed/\\1' frameborder=0 allowfullscreen></iframe>", $message);
				} elseif (strpos($message, 'tudou.com') !== FALSE) {
					 if (strpos($message, '/l/') !== FALSE) {
					 	$message = preg_replace("/\[flash=[\w,]+\]\s*.+\/l\/([\w\-]+)\/.*\s*\[\/flash\]/is", "<a href='http://www.tudou.com/listplay/\\1/index.html'><img src='static/image/common/vd.gif' /></a>", $message);
					 } elseif (stripos($message, '/v/') !== FALSE) {
					 	$message = preg_replace("/\[flash=[\w,]+\]\s*.+\/v\/([\w\-]+)\/.*\s*\[\/flash\]/is", "<a href='http://www.tudou.com/programs/view/\\1/'><img src='static/image/common/vd.gif' /></a>", $message);
					 }
				} elseif (stripos($message, 'qiyi.com') !== FALSE) {
					$message = preg_replace("/\[flash=[\w,]+\]\s*.+\/(\w+)\.swf.*\s*\[\/flash\]/is", "<a href='http://www.iqiyi.com/\\1.html'><img src='static/image/common/vd.gif' /></a>", $message);
				} elseif (strpos($message, '163.com') !== FALSE) {
					$message = preg_replace("/\[flash=[\w,]+\]\s*.+vid=(\w+).*sid=(\w+).*coverpic=([\w:\/\.]+).*\s*\[\/flash\]/is", "<div style='position:relative;'><a href='http://v.163.com/paike/\\2/\\1.html'><img src='\\3' /><img src='static/image/common/vds.png' style='position:absolute; top:50%; left:100px' /></a></div>", $message);
				} elseif (stripos($message, 'qq.com') !== FALSE) {
					$message = preg_replace("/\[flash=[\w,]+\]\s*.+vid=(\w+).*\s*\[\/flash\]/is", "<iframe frameborder='0' height='260' width='100%' src='http://v.qq.com/iframe/player.html?vid=\\1&tiny=0&auto=0' allowfullscreen></iframe>", $message);
				} else {
					$message = preg_replace("/\[flash=[\w,]+\]\s*([^\[\<\r\n]+?)\s*\[\/flash\]/is", "<a href='\\1'><span style='color:red;font-weight:bold'>该视频来源不支持外链手机播放，请点击跳转到视频地址播放</span></a>", $message);
				}
			}
		}
	...
	if(!$bbcodeoff) {
		...
		//if(defined('IN_MOBILE') && !defined('TPL_DEFAULT') && !defined('IN_MOBILE_API')) { //Enicn_d 2017-05-22 修改移动版图片显示，以支持分享缩略图
    if($_G['setting']['mobile']['mobilesimpletype']) {
		...
		if(strpos($msglower, '[/img]') !== FALSE) {
			$message = preg_replace(array(
				...
				//(!defined('IN_MOBILE') ? "bbcodeurl('\\3', '<a href=\"{url}\" target=\"_blank\">{url}</a>')" : "bbcodeurl('\\3', '<a href=\"{url}\" target=\"_blank\"><img src=\"{url}\" onload=\"thumbImg(this)\" alt=\"\" width=\"300\" /></a>')"),
				//Enicn_d 修改 2015-08-06 删除width="300"
				(!defined('IN_MOBILE') ? "bbcodeurl('\\1', '<a href=\"{url}\" target=\"_blank\">{url}</a>')" : "bbcodeurl('\\1', '<a href=\"{url}\" target=\"_blank\"><img src=\"{url}\" onload=\"thumbImg(this)\" alt=\"\" /></a>')"),
				(!defined('IN_MOBILE') ? "bbcodeurl('\\3', '<a href=\"{url}\" target=\"_blank\">{url}</a>')" : "bbcodeurl('\\3', '<a href=\"{url}\" target=\"_blank\"><img src=\"{url}\" onload=\"thumbImg(this)\" alt=\"\" /></a>')"),
			) : array("bbcodeurl('\\1', '{url}')", "bbcodeurl('\\3', '{url}')")), $message);
		}//Enicn_d modified 2015-04-13 make image can be view under mobile browser
	}

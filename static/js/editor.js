...
function insertAttachimgTag(aid) {
	if(wysiwyg) {
		insertText('<img src="' + $('image_' + aid).src + '" border="0" aid="attachimg_' + aid + '" alt="" /><br><br>', false);
	} else {
...
function discuzcode(cmd, arg) {
	...
	} else if(cmd == 'format') {//添加自动排版
		var nowTxt = html2bbcode(editdoc.body.innerHTML);
		editdoc.body.innerHTML = bbcode2html(textToFormat(nowTxt));
		setEditorTip('自动排版完成');
	} else if(cmd == 'rst') {
		loadData();
		setEditorTip('数据已恢复');
...
/**
* 自动排版内容处理 stellar add 2015-5-28
*/
function textToFormat(value){
	var value = "\n" + value;
	value = value.replace(/　　/ig,"");
	value = value.replace(/ (\n)/ig, function(p, p1){return p1});
	value = value.replace(/(\n)\s+/ig, function(p, p1){return p1});
	value = value.replace(/\r\n/ig,"\n");
	value = value.replace(/\n\n/ig,"\n");
	value = value.replace(/\n\n/ig,"\n");
	value = value.replace(/\n\n/ig,"\n");
